...
public class TriggerStateService {
     ...
     */
    public int toggleAllTriggersMatching(String tenant, List<QueryFilter> filters, boolean disabled) {
     ...
     * @throws NotFoundException if no trigger can be found.
     */
    public void resetTrigger(final TriggerId triggerId) throws NotFoundException {
        getTriggerState(triggerId); // check if state exists.
        try {
     ...
     * @return the number of triggers successfully unlocked.
     */
    public int unlockAllTriggersMatching(String tenant, List<QueryFilter> filters) {
        Flux<TriggerId> flux = triggerRepository.find(tenant, filters).filter(TriggerState::getLocked).map(TriggerId::of);
