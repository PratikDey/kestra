...
public abstract class AbstractTriggerRepositoryTest {
    ...
    protected static TriggerState generateDefaultTrigger(String tenantId){
        return TriggerState.builder()
            ...
            .flowId("flowId")
            .nextEvaluationDate(ZonedDateTime.now())
            .workerId("workerId")
            .build();
    }
    ...
    @ParameterizedTest
    @ValueSource(strings = {"io.kestra.unittest.1", "my-flow-1", "my-trigger-1"})
    void shouldFindGivenFulltextSearchQuery(String query) {
        // GIVEN
        String tenant = TestsUtils.randomTenant(this.getClass().getSimpleName());
        
        triggerRepository.save(trigger(tenant).namespace("io.kestra.unittest.1").flowId("my-flow1").triggerId("my-trigger-1").build());
        triggerRepository.save(trigger(tenant).namespace("io.kestra.unittest.2").flowId("my-flow2").triggerId("my-trigger-2").build());
    ...
    void shouldGetResultsForFindTriggersEligibleForSchedulingGivenExecutionDate() {
        ...
        triggerRepository.save(trigger(tenant1).triggerId("B").locked(false).vnode(1).nextEvaluationDate(now.plusMinutes(5)).build());
        triggerRepository.save(trigger(tenant2).triggerId("C").locked(false).vnode(2).nextEvaluationDate(now.minusMinutes(5)).build());
        triggerRepository.save(trigger(tenant2).triggerId("D").locked(false).vnode(3).nextEvaluationDate(null).build());
        
        // WHEN
        List<TriggerState> results = triggerRepository.findTriggersEligibleForScheduling(now, Set.of(0, 1, 2, 3), false)
            .stream().filter(it -> Set.of(tenant1, tenant2).contains(it.getTenantId())).toList();
